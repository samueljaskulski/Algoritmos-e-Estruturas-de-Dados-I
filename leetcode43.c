...
char *multiply( char *num1, char *num2 ) {
    ...
    int tam_resultado = tamn1 + tamn2;
    
    int *vetor = (int *) calloc(tam_resultado, sizeof(int));
    if (vetor == NULL) {
        return NULL;
    }
    
    for (int i = tamn1 - 1; i >= 0; i--) {
        for (int j = tamn2 - 1; j >= 0; j--) {
            int prod = (num1[i] - '0') * (num2[j] - '0');
            int p1 = i + j;
            int p2 = i + j + 1;

            int soma = prod + vetor[p2];
    ...
    int k = 0;
    while (k < tam_resultado && vetor[k] == 0) {
        k++;
    }

    if (k == tam_resultado) {
        free(vetor);
        char *resposta = (char *) malloc(2 * sizeof(char));
        strcpy(resposta, "0");
        return resposta;
    }

    int tam_final = tam_resultado - k;
    char *resposta = (char *) malloc((tam_final + 1) * sizeof(char));
   
    int idx = 0;
    while (k < tam_resultado) {
        resposta[idx] = vetor[k] + '0';
        idx++;
        k++;
    }
    resposta[idx] = '\0';
